...
class _AliceCallsListScreenState extends State<AliceCallsListScreen> with SingleTickerProviderStateMixin {
  ...
  Widget _buildTitleWidget() {
    return const Text('Draughtbeer Alice');
  }
    ...
    return StreamBuilder<List<AliceHttpCall>>(
      ...
      builder: (context, snapshot) {
        ...
        if (query.isNotEmpty) {
          calls = calls
              .where(
                (call) => call.endpoint.toLowerCase().contains(query.toLowerCase()),
              )
              .toList();
        }
        if (calls.isNotEmpty) {
      ...
      case AliceSortOption.responseTime:
        if (_sortAscending) {
          callsSorted
            ..sort()
            ..sort(
              (call1, call2) => call1.response?.time.compareTo(call2.response!.time) ?? -1,
            );
        } else {
          callsSorted.sort(
            (call1, call2) => call2.response?.time.compareTo(call1.response!.time) ?? -1,
          );
        }
      case AliceSortOption.responseCode:
        if (_sortAscending) {
          callsSorted.sort(
            (call1, call2) => call1.response?.status?.compareTo(call2.response!.status!) ?? -1,
          );
        } else {
          callsSorted.sort(
            (call1, call2) => call2.response?.status?.compareTo(call1.response!.status!) ?? -1,
          );
        }
      case AliceSortOption.responseSize:
        if (_sortAscending) {
          callsSorted.sort(
            (call1, call2) => call1.response?.size.compareTo(call2.response!.size) ?? -1,
          );
        } else {
          callsSorted.sort(
            (call1, call2) => call2.response?.size.compareTo(call1.response!.size) ?? -1,
          );
        }
                ...
                return Wrap(
                  children: [
                    ...AliceSortOption.values.map(
                      (AliceSortOption sortOption) => RadioListTile<AliceSortOption>(
